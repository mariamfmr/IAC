...
tabela_interrupções:      
    ...
    WORD rot_int_bombas                      ; interrupção 1 
...
POS_BOMBA_2:                            ; variavel que armazena a posição atual
	...
	WORD MEIO							; valor inicial da direção da bomba 2
...
POS_MISSIL:                            ; variavel que armazena a posição atual
	...
	WORD 0								; valor associado à direção do missil
...
INT_ENERGIA:                            ; variável que indica se se tem de
    WORD 0                              ; diminuir a energia da nave
			                            ; 1 - sim, 0 - não

INT_BOMBAS: WORD 1               ; variável que indica se
                                ; as bombas se têm de mover
                                ; 1 - sim, 0 - não
...
comandos_decorrer_jogo:
	...
	JZ move_missil			            ; se forem iguais, move o missil
	...
	JZ reduzir_energia                  ; se forem iguais, diminui a energia
...
move_bombas:
    ...
    MOV R0, INT_BOMBAS    			; verifica se houve interrupção para reduzir energia da nave
	MOV R0, [R0]
	CMP R0, 0
...
; ******************************************************************************
testa_limites_bomba:
; testa limites laterais esquerdos para as bombas que vão nessa direção
	MOV R6, ESQUERDA
	CMP R6, R3
	JNZ limites_direitos
	MOV R5, 30
	CMP R5, R1
	JZ reset_bomba			; caso tenha ultrapassado, dá reset à posição da bomba
	MOV R5, 33
	CMP R5, R2
	JZ reset_bomba
	JMP fim_limites_bomba

limites_direitos:				; limites de bombas que caminham na direção direita
	MOV R6, DIREITA
	CMP R6, R3
	JNZ limites_meio
	MOV R5, 60
...
comeca_jogo:
	...
	CALL desenha_bomba_1			    ; desenha a bomba no ecrã
